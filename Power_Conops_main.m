...
for i = length(trek_phase1)+1:length(time_vector)
    ...
    if (rock_found && can_avoid_rock)    
        ...
        curr_load_in_2 = (extreme_rove(2)*power_fraction_site2/3600); 
        energy_change = curr_load_in + -1 *(avoidance_consumption + avionics_consumption);    
        energy_change_2 = curr_load_in_2 + -1 *(avoidance_consumption + avionics_consumption);  
        temp_cap = battery_cap(i-1) + energy_change;
        temp_cap_2 = battery_cap_2(i-1) + energy_change_2;
        ...
        if (abs(temp_cap_2/battery_total - 1) > 1e-2)
            battery_cap_2(i) = battery_cap_2(i-1) + energy_change_2;
        else
    ...
    elseif (is_avoiding_rock)
        if (time_avoiding_rock == rock_avoidance_duration-1)
            ...
            curr_net_power_2 = nom_rove(2)*power_fraction_site2 - nom_rove(1);
            energy_change = curr_net_power / 3600; %[J/s] * 1Wh/3600J = Wh/s
            energy_change_2 = curr_net_power_2 / 3600; 
            temp_cap = battery_cap(i-1) + energy_change;
            temp_cap_2 = battery_cap(i-1) + energy_change_2;
